...
func (c *netFD) connect(ctx context.Context, la, ra syscall.Sockaddr) (rsa syscall.Sockaddr, ret error) {
	...
	case WSAEWOULDBLOCK, WSAEINPROGRESS:
	case nil, WSAEALREADY:
		select {
		case <-ctx.Done():
	...
	for {
		...
		errout := -1
		len := unsafe.Sizeof(errout)
		nerr, _, err := getsockoptProc.Call(uintptr(c.fd), uintptr(syscall.SOL_SOCKET), SO_ERROR, uintptr(unsafe.Pointer(&errout)), uintptr(unsafe.Pointer(&len)))
		if nerr != 0 {
