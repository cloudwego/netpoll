...
var climiter *concurrencyLimit

func enableConcurrencyLimit(limit int) {
	climiter = NewConcurrencyLimiter(limit, true)
	runTask = func(ctx context.Context, f func()) {
		climiter.Acquire(ctx)
		go func() {
			f()
			climiter.Release(ctx)
		}()
	}
}
...
func NewConcurrencyLimiter(limit int, blocking bool) *concurrencyLimit {
