...
func (p *defaultPoll) Control(operator *FDOperator, event PollEvent) (err error) {
	var op int
	var evt = new(syscall.EpollEvent)
	...
	case PollDetach:
		...
		op, evt = syscall.EPOLL_CTL_DEL, nil
	}
	err = syscall.EpollCtl(p.fd, op, operator.FD, evt)
